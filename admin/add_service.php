...
<div class="container mt-5">
    <div class="row">
        ...
        <h1>Ajouter un Service</h1>
                ...
                <div class="card-body">
        <form method="POST" action="process_service.php">
            ...
            <div class="form-group">
                ...
                <select id="cellule" name="cellule_id" required>
                <?php
                    require 'db.php';
                    $query = $db->query("SELECT id, name FROM structure WHERE hierarchy_level = 'Cellule'");
                    
                    if (!$query) {
                        die("Erreur de requête SQL : " . implode(":", $db->errorInfo()));
                    }

                    while ($cellule = $query->fetch()) {
                        echo "<option value='{$cellule['id']}'>{$cellule['name']}</option>";
                    }
                    ?>
                </select>
            </div>
