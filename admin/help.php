  ...
  <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
    ...
    <div class="app-wrapper">
	    ...
	    <div class="app-content pt-3 p-md-3 p-lg-4">
			...
			<h1 class="app-page-title">Formulaire de publication</h1>

<form action="artc.php" method="post" enctype="multipart/form-data">
    <div class="mb-3">
        <label for="titre" class="form-label">Titre</label>
        <input type="text" class="form-control" id="titre" name="Titre_annonce" required>
    </div>
    <div class="mb-3">
        <label for="contenu" class="form-label">Contenu</label>
        <textarea class="form-control" id="contenu" name="Description_annonce" required></textarea>
    </div>
    <div class="mb-3">
        <label for="document" class="form-label">Document</label>
        <input type="file" class="form-control" id="document" name="Document_annonce" required>
    </div>
    <div class="mb-3">
        <label for="date" class="form-label">Date</label>
        <input type="date" class="form-control" id="date" name="Date_annonce" required>
    </div>
    <button type="submit" class="btn btn-primary" value="submit">Envoyer</button>
</form><br><br>


<h1 class="app-page-title">Communiquer Publier</h1>  
<div class="row row-cols-1 row-cols-md-3 g-4">
    <?php
    $recupArticles = $db->query('SELECT * FROM annonces');
    while($article = $recupArticles->fetch()){
    ?>
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title"><?= htmlspecialchars($article['Titre_annonce']); ?></h5>
                    <p class="card-text"><?= htmlspecialchars($article['Description_annonce']); ?></p>
                    <a href="<?= htmlspecialchars($article['Document_annonce']); ?>" download class="btn btn-outline-primary" target="_blank">
                        Télécharger le document
                    </a>
                    <a href="supprimer_article.php?Id_annonce=<?= htmlspecialchars($article['Id_annonce']); ?>">
